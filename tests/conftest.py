...
def bq_repository():
    ...
    bq_repository.cashflow_source = f"SELECT * FROM {os.environ['DATASET']}.{os.environ['SOURCE_TABLE']}"
    bq_repository.irr_destination = os.environ['DATASET'] + '.' + os.environ['DESTINATION_TABLE']
...
def repository_with_cashflows(bq_repository):
    ...
    bq_repository.load_table_from_json(CASHFLOWS, os.environ['DATASET'] + '.' + os.environ['SOURCE_TABLE'], job_config)

    yield bq_repository

    bq_repository.client.delete_table(os.environ['DATASET'] + '.' + os.environ['SOURCE_TABLE'])
    bq_repository.client.delete_table(os.environ['DATASET'] + '.' + os.environ['DESTINATION_TABLE'])
